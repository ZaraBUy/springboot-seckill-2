...
public class UserService {
    ...
    public User getById(long id) {
        //对象缓存
        User user = redisService.get(UserKey.getById, "_" + id, User.class);
        if (user != null) {
            return user;
        }
        //取数据库
        ...
        //再存入缓存
        if (user != null) {
            redisService.set(UserKey.getById, "_" + id, user);
        }
        return user;
    }
    ...
    public boolean updatePassword(String token, long id, String formPass) {
        ...
        //更新缓存：先删除再插入
        redisService.delete(UserKey.getById, "_"+id);
        user.setPassword(toBeUpdate.getPassword());
        redisService.set(UserKey.token, "_" + token, user);
        return true;
    }
    ...
    public User getByToken(HttpServletResponse response, String token) {
        ...
        User user = redisService.get(UserKey.token, "_" + token, User.class);
